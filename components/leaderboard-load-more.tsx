      ...
      <ol className="leaderboard-table">
        {visible.map((row) => (
          <li className="leaderboard-row" key={row.id}>
            ...
            <Link className="lr-ava-wrap" href={`/clippers/${row.handle}?returnTo=${encodeURIComponent(returnTo)}`}><img src={row.avatar} alt="" loading="lazy" /></Link>
            <div className="lr-id"><Link href={`/clippers/${row.handle}?returnTo=${encodeURIComponent(returnTo)}`}><strong>{row.id === currentUserId ? "Я" : row.name}</strong></Link>{row.verified ? <BadgeCheck size={14} className="verified" /> : null}<LeagueBadge views={row.lifetimeViews} size="sm" /></div>
