...
export function LeaderboardLoadMore({ rows, clientMode }: { rows: Row[]; clientMode: boolean }) {
      ...
      <ol className="leaderboard-table">
        {visible.map((row) => (
          <li className="leaderboard-row" key={row.id}>
            ...
            <div className="lr-id"><strong>{row.name}</strong>{row.verified ? <BadgeCheck size={14} className="verified" /> : null}<LeagueBadge views={row.lifetimeViews} size="sm" /></div>
            ...
            ) : <Link className="invite-btn" href={`/clippers/${row.handle}`}><Handshake size={14} /><span className="invite-full">Открыть профиль</span><span className="invite-short">Открыть</span></Link>}
          </li>
        ))}
      </ol>
